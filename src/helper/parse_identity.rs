...
use age::{x25519, Identity, IdentityFile, IdentityFileEntry, Recipient};
    ...
    pub fn parse(
        ...
        if identity.is_empty() {
            ...
            let ident_file_entry = {
                if let Ok(i) = IdentityFile::from_file(identity.clone())
                    .map_err(|_| eyre!("import identity from file error"))
                    .and_then(|i| {
                        // TODO: replace with IdentityFile<callback> and into_ident into_recp
                        i.into_identities()
                            .get(0)
                            .cloned()
